...
def test_coach_and_certify_flow(tmp_path, monkeypatch):
    ...
    profile.save()
    profile.training_progress["rev"] = "in_progress"
    ...
    final = AgentIdentity.load("demo")
    assert any(c.get("id") == "rev" for c in final.certified_skills)
