...
class TestGPUManager(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.config = GPUConfig(
            mode=GPUMode.SINGLE, devices=[0], memory_fraction=0.5, mixed_precision=True
        )
    ...
    def test_memory_optimization(self):
        ...
        self.assertGreater(torch.cuda.memory_allocated(), initial_memory)
    ...
    def test_memory_profiling(self):
        ...
        profile = self.manager.profile_memory(
            self.manager.prepare_model(model), input_tensor.cuda()
        )
    ...
    def test_multi_gpu_setup(self):
        ...
        config = GPUConfig(
            mode=GPUMode.DATA_PARALLEL, devices=[0, 1], memory_fraction=0.5
        )
    ...
    def test_memory_stats(self):
        ...
        # Check memory increased
        self.assertGreater(
            new_stats["allocated"][0]["bytes"], stats["allocated"][0]["bytes"]
        )
    ...
    def test_error_handling(self):
        ...
        with self.assertRaises(Exception):
            config = GPUConfig(mode=GPUMode.SINGLE, devices=[100])  # Invalid device
            GPUManager(config)
