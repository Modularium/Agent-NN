...
class OpenHandsMonitoring(LoggerMixin):
    ...
    METRICS = {
        ...
        "cpu_usage": prom.Gauge(
            "openhands_cpu_usage_percent",
            "CPU usage percentage"
        ),
        "memory_usage": prom.Gauge(
            "openhands_memory_usage_bytes",
            "Memory usage in bytes"
        ),
        "disk_usage": prom.Gauge(
            "openhands_disk_usage_bytes",
            "Disk usage in bytes"
        ),
        
        # Docker metrics
        "container_count": prom.Gauge(
            "openhands_container_count",
            "Number of running containers",
            ["status"]
        ),
        "image_count": prom.Gauge(
            "openhands_image_count",
    ...
    async def get_metrics_summary(self) -> Dict[str, Any]:
        ...
        try:
            return {
                ...
                "docker": {
                    "containers": {
                        status: self.METRICS["container_count"].labels(status=status)._value.get()
    ...
    def _calculate_success_rate(self) -> float:
        ...
        try:
            ...
            for lang in ["python", "javascript", "typescript"]:
                ...
                completed = self.METRICS["executions_total"].labels(
                    language=lang,
                    status="completed"
                )._value.get()
                
                # Get failed executions
                failed = self.METRICS["executions_total"].labels(
                    language=lang,
                    status="failed"
                )._value.get()
                
                # Get error executions
                error = self.METRICS["executions_total"].labels(
                    language=lang,
                    status="error"
                )._value.get()
        ...
        try:
            ...
            for endpoint in ["/execute", "/docker", "/compose"]:
                for method in ["GET", "POST"]:
                    ...
                    success = self.METRICS["api_requests"].labels(
                        endpoint=endpoint,
                        method=method,
                        status=200
                    )._value.get()
                    ...
                    # Get client errors
                    client_errors = self.METRICS["api_requests"].labels(
                        endpoint=endpoint,
                        method=method,
                        status=400
                    )._value.get()
                    ...
                    # Get server errors
                    server_errors = self.METRICS["api_requests"].labels(
                        endpoint=endpoint,
                        method=method,
                        status=500
                    )._value.get()
