...
class TestMetaLearner(unittest.TestCase):
    ...
    def test_score_agents(self):
        ...
        scores = self.meta_learner.score_agents(
            self.task_embedding,
            agents_features
        )
        ...
        self.assertTrue(
            all(scores[i].combined_score >= scores[i+1].combined_score
    ...
    def test_historical_performance(self):
        ...
        # Add some metrics
        metrics = [
            TaskMetrics(response_time=0.5, confidence_score=0.8,
                       user_feedback=4.5, task_success=True),
            TaskMetrics(response_time=0.7, confidence_score=0.6,
                       user_feedback=3.5, task_success=False)
        ]
