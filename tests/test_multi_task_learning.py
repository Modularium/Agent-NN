...
class TestMultiTaskLearning(unittest.TestCase):
    def setUp(self):
        ...
        self.inputs = {
            "classification": torch.randn(
                self.batch_size,
                self.task_configs["classification"]["input_dim"]
            ),
            "regression": torch.randn(
                self.batch_size,
                self.task_configs["regression"]["input_dim"]
            )
        }
        ...
        self.targets = {
            "classification": torch.randint(
                0,
                self.task_configs["classification"]["output_dim"],
                (self.batch_size,)
            ),
            "regression": torch.randn(
                self.batch_size,
                self.task_configs["regression"]["output_dim"]
            )
        }
    ...
    def test_attention_fusion(self):
        ...
        features = [
            torch.randn(self.batch_size, 256)
            for _ in range(3)
        ]
