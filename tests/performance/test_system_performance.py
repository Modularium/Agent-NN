    ...
    def get_gpu_metrics(self) -> Dict[str, List[Dict[str, float]]]:
        ...
        try:
            ...
            for handle in self.gpu_handles:
                ...
                metrics["utilization"].append({"gpu": util.gpu, "memory": util.memory})
                ...
                metrics["temperature"].append({"gpu": temp})

                # Get power usage
                try:
                    power = pynvml.nvmlDeviceGetPowerUsage(handle) / 1000.0  # W
                    metrics["power"].append({"usage": power})
                except:
                    metrics["power"].append({"usage": 0})
...
class TestSystemPerformance(unittest.TestCase, LoggerMixin):
    ...
    def setUpClass(cls):
        ...
        cls.cache_manager = CacheManager(max_size=1024, cleanup_interval=1)  # 1GB
    ...
    async def test_concurrent_model_loading(self):
        ...
        for i in range(num_models):
            ...
            models.append({"name": f"model_{i}", "path": path})
        ...
        # Log results
        self.log_event("model_loading_test", self.metrics.get_summary())
    ...
    async def test_cache_performance(self):
        ...
        # Log results
        self.log_event("cache_performance_test", self.metrics.get_summary())
    ...
    async def test_knowledge_base_search(self):
        ...
        await self.knowledge_manager.create_knowledge_base(
            name=kb_name, domain="test", sources=[]
        )
        ...
        async def add_document(i: int):
            ...
            return await self.knowledge_manager.process_document(
                kb_name, f"doc_{i}.txt", content.encode()
            )
        ...
        async def search_kb():
            ...
            try:
                results = self.knowledge_manager.search_knowledge_base(
                    kb_name, "test document", limit=10
                )
                latency = time.time() - start
                self.metrics.add_latency(latency)
        ...
        # Log results
        self.log_event("kb_search_test", self.metrics.get_summary())
    ...
    async def test_system_backup_restore(self):
        ...
        # Log results
        self.log_event("backup_restore_test", self.metrics.get_summary())
    ...
    def test_system_stress(self):
        ...
        def stress_worker():
            try:
                ...
                while time.time() - self.metrics.start_time < duration:
                    ...
                    # Random operation
                    op = np.random.choice(["cache", "model", "knowledge", "system"])
                    ...
                    if op == "cache":
                        ...
                        # Model operation
                        model = torch.nn.Linear(10, 1)
                        path = os.path.join(self.model_dir, f"stress_{time.time()}.pt")
                        torch.save(model.state_dict(), path)
        ...
        # Log results
        self.log_event("stress_test", self.metrics.get_summary())
