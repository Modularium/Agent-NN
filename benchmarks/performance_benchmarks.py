...
class Benchmarker(LoggerMixin):
    ...
    def benchmark_throughput(self,
                           model: nn.Module,
                           input_fn: Callable[[int], torch.Tensor]) -> BenchmarkResult:
    ...
    def benchmark_scaling(self,
                        model: nn.Module,
                        input_fn: Callable[[int], torch.Tensor]) -> Dict[str, BenchmarkResult]:
    ...
    def profile_memory_usage(self,
                           model: nn.Module,
                           input_fn: Callable[[int], torch.Tensor]) -> Dict[str, Any]:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                nested = self._flatten_metrics(
                    value,
                    f"{prefix}{key}_" if prefix else f"{key}_"
                )
