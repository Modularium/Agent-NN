...
class TestABTesting(unittest.TestCase):
    ...
    def test_test_creation(self):
        ...
        # Create test
        test = self.manager.create_test("test_1", self.variants, self.metrics)
        ...
        invalid_variants = [Variant("a", None, {}, 0.6), Variant("b", None, {}, 0.6)]
        with self.assertRaises(ValueError):
            self.manager.create_test("test_2", invalid_variants, self.metrics)

    def test_test_lifecycle(self):
        """Test A/B test lifecycle."""
        # Create and start test
        test_id = "test_1"
        test = self.manager.create_test(test_id, self.variants, self.metrics)
        ...
        for _ in range(5):
            self.manager.add_result(
                test_id, "variant_a", {"accuracy": 0.8, "loss": 0.2}
            )
            self.manager.add_result(
                test_id, "variant_b", {"accuracy": 0.9, "loss": 0.1}
            )
    ...
    def test_result_analysis(self):
        ...
        test = self.manager.create_test(
            test_id, self.variants, self.metrics, min_samples=5
        )
        self.manager.start_test(test_id)
        ...
        for _ in range(3):
            self.manager.add_result(
                test_id, "variant_a", {"accuracy": 0.7, "loss": 0.3}
            )
            self.manager.add_result(
                test_id, "variant_b", {"accuracy": 0.9, "loss": 0.1}
            )
    ...
    def test_state_persistence(self):
        ...
        self.manager.create_test(test_id, self.variants, self.metrics)
        self.manager.start_test(test_id)

        # Add some results
        self.manager.add_result(test_id, "variant_a", {"accuracy": 0.8, "loss": 0.2})
        ...
        self.assertEqual(len(loaded_test.results["variant_a"]), 1)
    ...
    def test_test_completion(self):
        ...
        test = self.manager.create_test(
            test_id, self.variants, self.metrics, min_samples=2, max_duration=1  # 1 day
        )
        self.manager.start_test(test_id)
    ...
    def test_error_handling(self):
        ...
        # Test empty variants
        with self.assertRaises(ValueError):
            self.manager.create_test(test_id, [], self.metrics)  # Empty variants list
        ...
        with self.assertRaises(ValueError):
            self.manager.create_test(test_id, invalid_variants, self.metrics)
            self.manager.add_result(test_id, "unknown", {"accuracy": 0.8})
        ...
        with self.assertRaises(ValueError):
            self.manager.add_result(
                test_id, "unknown", {"accuracy": 0.8}  # Unknown variant
            )
    ...
    def test_duration_based_completion(self):
        ...
        test_id = "test_1"
        test = self.manager.create_test(
            test_id, self.variants, self.metrics, max_duration=1  # 1 day
        )
        self.manager.start_test(test_id)
        ...
        self.manager.add_result(test_id, "variant_a", {"accuracy": 0.8})
