...
def test_mission_progression(tmp_path, monkeypatch):
    ...
    profile = AgentIdentity.load("a1")
    assert profile.mission_progress["m1"]["step"] == 1
    assert "m1" in profile.active_missions

    service.dispatch_task(TaskContext(task_type="demo"), mission_id="m1", mission_step=1)
    profile = AgentIdentity.load("a1")
    assert profile.mission_progress["m1"]["status"] == "complete"
